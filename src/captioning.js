...
function heuristicImageTraits(typeKey, context = "", metadata = {}) {
  ...
  if (typeKey === "task_collab_chair") {
    ...
    inferred.configuration = /sofa|loveseat|settee/.test(source) ? "multi-seat / sofa" : /modular/.test(source) ? "modular component" : /corner/.test(source) ? "corner unit" : /chair|lounge/.test(source) ? "single seat" : "unknown";
...
async function extractStage23CombinedOpenAi(imageInput, typeKey, stage1, options = {}) {
  ...
  const { data: parsed, usage } = await callOpenAiJsonWithMeta({
    ...
    userParts: [
      ...
      {
        type: "input_text",
        text: `Stage 1 seating_type result: ${stage1.seating_type}. Use this as the routing type for all stage 2 and stage 3 outputs.`
      },
      { type: "input_image", image_url: imageInput.image_url, detail: "high" }
    ],
...
- lounge_chair: low or relaxed seating including single lounge chairs, sofas, modular lounge pieces, wrapped-shell lounge forms, pedestal lounge chairs, and high-back privacy chairs
...
Fields to extract:
- seating_type: task_collab_chair | lounge_chair | stool | guest_chair | bench | ottoman | other_seating
...
- lounge_chair: Lounge Chair. Lounge chairs, club chairs, accent chairs, sofas, modular lounge pieces, and privacy lounge seating for relaxed posture.
- bench: Bench. Benches and banquettes intended for multiple people on one continuous seat.
- ottoman: Ottoman. Backless upholstered footrests or occasional seats such as ottomans, poufs, and footstools.
...
export async function generateImageExtractionRecord(imageRecord = {}, options = {}) {
    ...
    return buildExcludedImageExtractionResult({
      ...
      tokens: {
        ...
        runs: [
          {
            run: run1.run_label,
            usage: run1.usage?.total || normalizeOpenAiUsage()
          }
        ],
        total: totalUsage
      },
      cost: {
        ...
        runs: [
          {
            run: run1.run_label,
            estimated_cost_usd: Number(run1.usage?.estimated_cost_usd || 0)
          }
        ],
  ...
  const derivedBaseMaterial = deriveBaseMaterialFromBaseFinish(voted.stage1?.seating_type || "", enumFields);
  if (derivedBaseMaterial) {
    enumFields.base_material = derivedBaseMaterial;
  }
  const derivedBaseVisibility = deriveBaseVisibilityFromBaseType(voted.stage1?.seating_type || "", enumFields);
  ...
  const searchText = buildSearchableText({
    ...
    brand: imageRecord.brand || "",
    seatingType: voted.stage1?.seating_type || "",
    enumFields,
    freeText
  });
